...
var createTableSQL = `CREATE TABLE IF NOT EXISTS chash (
...
func CreateTable() {
	...
	statement.Exec()
	log.Println("chash table created")
}

func InsertNote(word string, definition string, category string) {
	insertNoteSQL := `INSERT INTO chash (word, definition, category)
	VALUES (?, ?, ?)`
...
func DisplayAllNotes() {
	row, err := db.Query("SELECT * FROM chash ORDER BY word")
	if err != nil {
		log.Fatalln(err)
	}
